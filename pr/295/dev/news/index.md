...
- Add `kri0019` / `cou0019`: Time to First Adverse Event, the first
  `gsm.kri` metric to use `AnalysisType: poisson`
- Add
  [`Input_TimeToEvent()`](https://gilead-public.github.io/gsm.kri/dev/reference/Input_TimeToEvent.md),
  a metric-agnostic helper that derives a subject-level
  time-to-first-event `Analysis_Input`, right-censoring subjects with no
  event
- Add
  [`Report_TimeToAE()`](https://gilead-public.github.io/gsm.kri/dev/reference/Report_TimeToAE.md)
  and the `report_timetoae` module workflow, with a Kaplan-Meier
  event-free curve alongside the standard metric charts
- Add
