...
`Reporting_ActionLog` is a frozen extract taken when the snapshot is
...
`SnapshotDate`s. For each site and metric, the score uses the latest
entry on or before the results snapshot, or on or before
`dActionSnapshotDate` when that is supplied (it must not be later than
the results snapshot). Entries are matched to the KRI results on
`StudyID + GroupLevel + GroupID + MetricID`, and
`StudyID + SnapshotDate + GroupLevel + GroupID + MetricID` must be
unique across the extract. Scoring an earlier snapshot therefore uses
...
returns the per-KRI rows behind the score: the weight, the ActionLog
state and date applied, and the resulting effective weight. Passing it
to `Report_KRI(dfRiskScoreDetail = ...)` adds a badge to each adjusted
score showing how far it is below the Site Risk Score, and clicking an
adjusted score lists the KRIs responsible. Reports without
`Analysis_srs0002` are unchanged. The `srs0002.yaml` workflow reads
persisted `Analysis_Summary` and `Reporting_ActionLog`, which keeps ADO
credentials and extraction timing outside
...
When both metrics are present, the standard site-level
[`Report_KRI()`](https://gilead-public.github.io/gsm.kri/dev/reference/Report_KRI.md)
...
column. Reports without `Analysis_srs0002` retain the original layout.
Direct
[`Widget_GroupOverview()`](https://gilead-public.github.io/gsm.vizr/reference/Widget_GroupOverview.html)
callers can request the same layout with
`strComparisonRiskMetric = "Analysis_srs0002"`; leaving that argument
`NULL` preserves the single-score widget.
