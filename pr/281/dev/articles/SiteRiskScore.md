...
The
[`gsm.kri::CalculateRiskScore()`](https://gilead-public.github.io/gsm.kri/dev/reference/CalculateRiskScore.md)
...
The `srs0002` variant (**Adjusted Risk Score**) starts from the same KRI
flag weights as `srs0001` and multiplies each KRI’s contribution by a
factor taken from its Central Monitoring ActionLog state. The
denominator is unchanged, so a finding triaged as needing no action
stops counting toward the site’s score without shrinking the scale the
score is measured on.

#### Formula

For site $`s`$ at snapshot $`t`$, summing over the site’s KRI results
$`k`$:
...
- $`w_k(f)`$ is the weight for flag $`f`$ of KRI $`k`$ from
  [`MakeWeights()`](https://gilead-public.github.io/gsm.kri/dev/reference/MakeWeights.md),
  the same weight `srs0001` uses, and $`W^{\max}_k`$ is that KRI’s
  largest weight.
...
Every factor is between 0 and 1, so the Adjusted Risk Score is never
higher than the Site Risk Score; with all factors equal to 1 it matches
`srs0001` exactly. For example, with three KRIs weighted `4`, `4`, and
`8` in states `No Action`, `Open Action`, and `Closed Action`, the
numerator is `0 + 4 + 8 = 12` over the original denominator of `16`, a
score of `75`.
...
`Reporting_ActionLog` is a frozen extract taken when a snapshot is
generated, so it only contains decisions made up to that point. It keeps
a history: the same site and metric can have entries at several
`SnapshotDate`s.

- For each site and metric, the latest entry on or before the results
  `SnapshotDate` is used. `dActionSnapshotDate` sets an earlier “as of”
  date; it cannot be later than the results snapshot.
- Entries are matched to KRI results on
  `StudyID + GroupLevel + GroupID + MetricID`, and
  `StudyID + SnapshotDate + GroupLevel + GroupID + MetricID` must be
  unique across the extract.
- Scoring an earlier snapshot therefore uses only what was known at that
  point, and entries dated after the results snapshot are ignored.

Only the state of that one entry is used; earlier entries for the same
signal do not affect the score.

#### Missing action states

A KRI with a nonzero weight and no matching ActionLog entry is an error
by default (`strMissingState = "error"`). Choose `"include"` to count
its full weight until it is triaged, or `"exclude"` to drop it.
Zero-weight KRIs never need an ActionLog entry.

#### Workflow

The `srs0002.yaml` workflow reads the persisted `Analysis_Summary` and
`Reporting_ActionLog` domains, which keeps ADO credentials and
extraction timing outside
[gsm.kri](https://github.com/Gilead-Public/gsm.kri), and returns
`Analysis_srs0002` alongside `srs0001`. The original `srs0001` remains
unchanged as the comparison and rollback baseline.

#### Reporting

When both scores are present, the site-level
[`Report_KRI()`](https://gilead-public.github.io/gsm.kri/dev/reference/Report_KRI.md)
overview shows `Analysis_srs0002` in an **Adjusted Risk Score** column
immediately after the existing **Risk Score** (`Analysis_srs0001`)
column.

[`MakeActionRiskScoreDetail()`](https://gilead-public.github.io/gsm.kri/dev/reference/MakeActionRiskScoreDetail.md)
returns the per-KRI rows behind the score: each KRI’s weight, the
ActionLog state and date applied, and the resulting effective weight.
[`CalculateActionRiskScore()`](https://gilead-public.github.io/gsm.kri/dev/reference/CalculateActionRiskScore.md)
summarizes these same rows, so the two always agree. Passing them to
`Report_KRI(dfRiskScoreDetail = ...)` adds:

- a small “▼N” marker on each Adjusted Risk Score that is lower than the
  Risk Score, and
- a breakdown that opens under the site’s row when its Adjusted Risk
  Score is clicked, listing each KRI’s flag, action state, ActionLog
  date, and points, with KRIs that no longer count shown struck through.
...
Reports without `Analysis_srs0002` keep the original layout, and any
detail passed to them is ignored.
[`gsm.vizr::Widget_GroupOverview()`](https://gilead-public.github.io/gsm.vizr/reference/Widget_GroupOverview.html)
callers can request the same layout with
`strComparisonRiskMetric = "Analysis_srs0002"` and
`dfComparisonRiskDetail`; leaving `strComparisonRiskMetric` `NULL`
...
The
[`Widget_CrossStudyRiskScore()`](https://gilead-public.github.io/gsm.kri/dev/reference/Widget_CrossStudyRiskScore.md)
...
2.  **Summary generation**: Use
    [`SummarizeCrossStudy()`](https://gilead-public.github.io/gsm.kri/dev/reference/SummarizeCrossStudy.md)
