...
generated, so it only contains decisions made up to that point, and it
can span several `SnapshotDate`s. By default the score uses the rows at
the latest `SnapshotDate` in the ActionLog, which hold the current state
of every signal; `dActionSnapshotDate` selects a different date. That
date must not be later than the `dfResults` snapshot. The selected rows
are joined to the KRI results on
`StudyID + GroupLevel + GroupID + MetricID`, and must be unique per key
(`StudyID + SnapshotDate + GroupLevel + GroupID + MetricID` across the
whole extract). The `srs0002.yaml` workflow reads persisted
`Analysis_Summary` and `Reporting_ActionLog`, which keeps ADO
