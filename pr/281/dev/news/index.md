...
- Added
  ...
  and the `srs0002` workflow for an action-status-weighted Site Risk
...
- Added configurable risk-score MetricIDs to the cross-study summary and
  ...
  ([\#280](https://github.com/Gilead-Public/gsm.kri/issues/280)).
- Extended the existing site overview in
  [`Report_KRI()`](https://gilead-public.github.io/gsm.kri/dev/reference/Report_KRI.md)
  to display an adjusted Site Risk Score immediately after the original
  Risk Score when `Analysis_srs0002` is available. Direct
  [`Widget_GroupOverview()`](https://gilead-public.github.io/gsm.vizr/reference/Widget_GroupOverview.html)
  consumers retain the original single-score layout unless they opt in
  ([\#280](https://github.com/Gilead-Public/gsm.kri/issues/280)).
...
- `pd_BucketBar()` is removed
  ([\#264](https://github.com/Gilead-Public/gsm.kri/issues/264)) — the
  Plotly bucket bar, left without a caller once the Premature Deaths
  report moved to gsm.viz. Use
...
- The legacy widget wrappers (`Widget_BarChart`, `Widget_ScatterPlot`,
  ...
  and the shared widget-control JS/CSS moved to gsm.vizr
  ([\#291](https://github.com/Gilead-Public/gsm.kri/issues/291)).
  gsm.kri re-exports every name, so existing callers are unaffected. The
  vendored `gsm.viz-2.4.1` bundle is retired;
  `Widget_CrossStudyRiskScore` now takes the bundle from
  [`gsm.vizr::html_dependency_gsm_viz()`](https://gilead-public.github.io/gsm.vizr/reference/html_dependency_gsm_viz.html).
- The Premature Deaths bucket and reason barcharts render through
  [`gsm.vizr::bars()`](https://gilead-public.github.io/gsm.vizr/reference/bars.html)
  via the new
  [`pd_BucketBarSpec()`](https://gilead-public.github.io/gsm.kri/dev/reference/pd_BucketBarSpec.md)
  /
  ...
  ([\#264](https://github.com/Gilead-Public/gsm.kri/issues/264),
  [\#288](https://github.com/Gilead-Public/gsm.kri/issues/288)). The
  pilot wrappers `Widget_PrematureDeathBucketBar()` /
  `Widget_PrematureDeathReasonBar()` and their four Shiny bindings are
  removed; they were added after v1.6.1 and never shipped in a release.
  The hand-rolled count/percent toggle is replaced by gsm.viz’s native
  position toggle, and a missing `OuterGroupID` now serializes to a real
  `null` instead of the literal string `"NA"`.
- The Eligibility report’s 7 bar charts (Site, Country, Source, and the
  four Criteria/… tabs) render through gsm.qtl’s
