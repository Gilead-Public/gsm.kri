...
#'
#' htmlwidget wrapper that renders premature-death category counts via a flat
#' `gsm.viz` `bars` chart (one per study/country/site view). Serializes long rows
...
#' @param spec `list` Serializable `bars` spec from [pd_BucketBarSpec()].
...
Widget_PrematureDeathBucketBar <- function(
  ...
  pd_BarWidget("Widget_PrematureDeathBucketBar", data, spec, metadata, bDebug)
}
