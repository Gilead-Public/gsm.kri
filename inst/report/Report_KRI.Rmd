...
GroupOverViz <- Widget_GroupOverview(
  ...
  strGroupSubset = if (is.null(params$strGroupSubset)) {
    switch(setup$GroupLevel, Site = 'red', 'all')
  } else {
    params$strGroupSubset
  },
  strGroupLabelKey = switch(setup$GroupLevel,
    Site = 'InvestigatorLastName',
    NULL
  ),
  strComparisonRiskMetric = params$strComparisonRiskMetric,
  strComparisonRiskLabel = params$strComparisonRiskLabel
)
...
```{r srs-weighting-summary, echo=FALSE, results='asis'}
if (
  "Analysis_srs0001" %in% params$dfResults$MetricID &&
    all(c("MetricID", "Flag", "RiskScoreWeight") %in% names(params$dfMetrics))
) {
  Report_SRSWeighting(
    dfMetrics = params$dfMetrics,
    dfResults = params$dfResults
