...
# Transform
qualification_transform_counts <- function(
  dfInput,
  countCol = "Count",
...
qualification_flag_poisson <- function(
  dfAnalyzed,
  threshold = c(-7, -5, 5, 7)
) {
  dfAnalyzed %>%
  ...
  if (strType == "binary") {
    scored <- dfTransformed %>%
      mutate(
        ...
        z_0 = ifelse(
          OverallMetric == 0 | OverallMetric == 1,
          0,
        ...
        Factor = mean(z_0^2),
        Score = ifelse(
          OverallMetric == 0 | OverallMetric == 1 | Factor == 0,
          0,
          ((Metric - OverallMetric) /
...
qualification_flag_normalapprox <- function(
  dfAnalyzed,
  threshold = c(-3, -2, 2, 3)
) {
  dfAnalyzed %>%
...
qualification_flag_identity <- function(
  dfAnalyzed,
  threshold = c(0.000895, 0.003059)
) {
  dfAnalyzed %>%
    mutate(
      Flag = case_when(
