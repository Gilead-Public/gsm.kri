...
The join key is `StudyID + SnapshotDate + GroupLevel + GroupID + MetricID`.
`Reporting_ActionLog` must therefore be refreshed and reduced to one row per key
before the metric workflow runs. The `srs0002.yaml` workflow reads persisted
`Analysis_Summary` and `Reporting_ActionLog`, which keeps ADO credentials and
extraction timing outside `{gsm.kri}`.
